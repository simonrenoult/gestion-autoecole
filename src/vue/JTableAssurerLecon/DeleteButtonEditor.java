...
public class DeleteButtonEditor extends DefaultCellEditor
{
	...
	class DeleteButtonListener implements ActionListener
	{
		...
		public boolean verifierDroitSuppressionDate()
		{
			 ...
			 * 
			 * // On gere les 4 cas possibles pour les dates et heures selon les
			 * formats mis en place.
			 * 
			 * try { dateComparee =
			 * dateFormat.parse(String.valueOf(table.getModel
			 * ().getValueAt(table.getSelectedRow(), 0))); } catch
			 * (ParseException e) { dateComparee.setYear(3000); }
			 ...
			 * try { heureComparee =
			 * heureFormat.parse(String.valueOf(table.getModel
			 * ().getValueAt(table.getSelectedRow(), 1))); } catch
			 * (ParseException e) {
			 ...
			 * //System.out.println("dateActuelle : "+dateActuelle.toString());
			 * //System.out.println("dateComparee : "+dateComparee.toString());
			 * if(dateComparee.compareTo(dateActuelle) <= 0){ index = false;
			...
			System.out.println("date comparee : " + Integer.parseInt(dateComparee[0])
					+ Integer.parseInt(dateComparee[1]) + Integer.parseInt(dateComparee[2]));
			System.out.println("dateactuelle : " + Integer.parseInt(DateACTUELLE[0])
		...
		public void actionPerformed(ActionEvent event)
		{
			if (table.getRowCount() > 0)
			{
				...
				int option = new JOptionPane().showConfirmDialog(null ,
						"Voulez-vous supprimer ce RDV (opération irréversible) ?" , "Suppression de RDV" ,
				...
				if (option == JOptionPane.OK_OPTION)
				{
					...
					else
					{
						...
						new JOptionPane()
								.showConfirmDialog(
										null ,
										"Par raison de sécurité, vous ne pouvez pas supprimer un RDV\n"
