...
/*
 * Cette classe permet de d'extraire les données de la JTable (conversion d'un tableau
 * d'objet [][] en Liste KAssurerLecon). Cette liste va être ensuite comparée à la liste de 
 * référence KlisteRDVRef afin de voir ci des RDV ont été modifié. La table Agenda admet un id numérique
 * et non plus une date, donc les on compare aussi les dates et heures de RDV entre ceux de la Jtable
 * KlisteAgendaGraphique et la liste de reference KlisteAgendaRef (par rapport à un élève). Cette necessité 
 * de ces listes provient du fait qu'on ne renseigne pas de dates, ni d'heure dans la liste KlisteRDVRef.
 * 
 * Exemple : 
 * 
 * KlisteAgendaRef
 ...
 * 
 * 17	2011-08-24	15:30:00		similaire celle de la BDD, update en BDD 
 * 18	2011-08-25	15:30:00		n'existe pas en BDD, Insert
 * -1	1899-12-31	00:00:00		erreur de format dans le champ grahique du 3eme RDV (3eme ligne de la Jtable), génère un message d'erreur.
 * 
 * TableauChampSaisieOk est un tableau de booléen où chaque champ détermine si le format des données 
 * est valide en BDD.
 * 
 */
...
////////////////////////////////////////////////////////////////
	...
	public int AjoutAgenda (KAgenda agenda,int idMoniteur){
		...
		//KAgenda agenda1 = new KAgenda();
		int id = recupererDerniereCle(idMoniteur);
		//agenda1.setId(id);
		//agenda1.setDATE_AGENDA(agenda.getDATE_AGENDA());
		//agenda1.setHEURE_AGENDA(agenda.getHEURE_AGENDA());
		
		//System.out.println(agenda1.add(bdd.db));
		...
		try {
			...
			statement.executeUpdate("INSERT INTO Agenda VALUES ("+id+",'"+agenda.getDATE_AGENDA()+"','"+agenda.getHEURE_AGENDA()+"' )");
	...
	private int recupererDerniereCle(int idMoniteur) {
		...
		agenda.loadFromDb(bdd.db,"select * from assurer_lecon where idmoniteur = "+idMoniteur+" order by idAgenda asc");
...
////////////////////////////////////////////////////////////////
               //Concernant la table Assurer_Lecon
////////////////////////////////////////////////////////////////
	 ...
	 * la BDD pour ce meme eleve. le point de départ est donc KlisteRDVRef.count() pour 
	 * recuperer juste les nouvelles leçons.
	 */
	...
	/*
	 * Recupere la liste des RDV d'un eleve pour lequel il y a un equivalent en BDD. Tout est alors
	 * stocké dans KlisteRDVRef.
	 */
	...
	public KListObject<KAssurer_lecon> convertirTableau (Object[][] Tab,int idEleve){
		...
		KAgenda agenda;
		KlisteAgendaGraphique.clear();//efface la collection pour en refaire une
	...
	/*
	 * teste l'ensemble de la liste "graphique" pour tester les formats de données, cohérence de données.
	 */
	@SuppressWarnings("deprecation")
	public int gererRDVvide(KListObject<KAssurer_lecon> klisteGraphique) {
		
		java.sql.Date dateActuelle = new java.sql.Date(1, 0, 0);//1900-12-31 00:00:00.0 !!
...
///////TRAITEMENT DES DONNES (VERIFICATION FORMAT, ETC...).
	/*
	...
	public String messageRenvoyeeUI (int num){
		...
		case 2 : return "-L'ensemble des RDV ont été enregistrés en base de données\n";
		
		case 1 : return "";
		
		case -1 : return "-La date de RDV fixée ne peut pas être antérieur à la date actuelle\n";
		
		case -2 : return "-L'horaire saisie est invalide : les heures sont compris entre 08h et 19h\n";
		...
		case -5 : return "-Une durée de leçon est invalide (expression absente ou format invalide).\n";
		...
		case -8 : return "-Un horaire de leçon est invalide (expression absente ou format invalide).\n";
	...
	/*
	 * Creation d'un objet java.sql.TimeStamp à partir de java.sql.Date et java.sql.Ime
	 */
	@SuppressWarnings("deprecation")
...
@SuppressWarnings("deprecation")
	...
	/*
	 * Retourne un indice pour voir si le format de la date et heure sont valide (ex : 2011-12-ertl) --> -1.
	 */
	@SuppressWarnings("deprecation")
	...
	public KAssurer_lecon contrainteintegriteMoniteur(KListObject<KAssurer_lecon> KlisteGraphique ){
		...
		//doit se faire sur toute la base ne filtrant sur la date et le moniteur
		...
		for ( i = 0; i< KlisteGraphique.count(); i++){
			...
			if(Kliste.count() != 0){
				return Kliste.get(0);// On prend chaque RDV qui cloche et on envoie un message d'erreur.
			}
		}
		
		return new KAssurer_lecon();//On retourne un RDV vide, temoin que ça s'est bien passé.
	...
	public int contrainteintegriteEleve(KListObject<KAssurer_lecon> KlisteGraphique){
		...
		for ( i = 0; i< KlisteGraphique.count(); i++){
			...
			//select * from assurer_lecon where idAgenda in (select id from agenda where date_agenda = '2011-11-15' and heure_agenda between '08:00:00' and '09:00:00') and idEleve = 5
			...
			if(Kliste.count() !=0){
				System.out.println("");
				return KlisteAgendaRef.count()+i;// numero de ligne où ça cloche.
			}
