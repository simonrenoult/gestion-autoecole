		...
		if (erreur != 1){
			...
			if (erreur != 1){
				...
				if(rdv !=-1){
					...
					//6
					fenetre.getFicheEleve().getTableauLecon().creerClePrimaireTableAgendaEtInsertionRDVnouveau(KlisteNouveauRDV);
					//7)
					fenetre.getFicheEleve().getTableauLecon().MAJRDV(KlisteRDVexistant);
					//8)
					
					JOptionPane.showMessageDialog(null,fenetre.getFicheEleve().getTableauLecon().messageRenvoyeeUI(2)
